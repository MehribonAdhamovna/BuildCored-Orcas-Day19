"""
BUILDCORED ORCAS — Day 19: I2CPlayground
Complete I2C protocol simulation with animated waveforms.

Hardware concept: I2C Bus Protocol
I2C uses just 2 wires (SDA + SCL) to connect hundreds of
sensors to a microcontroller. After today you can read any
sensor datasheet and understand the timing diagrams.
...
- ACK bit: 9th bit after each byte. LOW=ACK, HIGH=NACK
- START: SDA goes LOW while SCL is HIGH
- STOP:  SDA goes HIGH while SCL is HIGH

YOUR TASK:

1. Add a NACK simulation (TODO #1)
1. Implement clock stretching (TODO #2)
1. Run a multi-byte transaction (TODO #3)

Run: python day19_starter.py
"""
...
def simulate_nack():
    """
    Simulates a transaction where the slave does not exist (0x99).
    Forces a NACK after the address byte to show a 'device not found' error.
    """
    print("Simulating NACK: Device at 0x99 not found...")
   
    segments = encode_i2c_transaction(0x99, 0x00, [0x00])
    
    nack_sda, nack_scl = generate_ack(ack=False)
    segments[2] = ("NACK", nack_sda, nack_scl, "NACK (No Device)")
    
    print_transaction(segments)
    
    animate_transaction(segments, title="I2C Error: NACK (Device Not Found)")
...
def simulate_clock_stretching():
    """
    Simulates clock stretching by holding SCL LOW for an extended period.
    """
    print("Simulating Clock Stretching...")
    segments = encode_i2c_transaction(0x48, 0x1A, [0x42])
    

    label, sda, scl, ann = segments[2]
    
    stretched_scl = [0] * 40 + scl
    stretched_sda = [0] * 40 + sda 
    
    segments[2] = ("STRETCH", stretched_sda, stretched_scl, "STRETCH (Slave Busy)")
    
    print_transaction(segments)
    animate_transaction(segments, title="I2C: Clock Stretching")
...
def run_mpu6050_transaction():
    print("Simulating MPU6050 14-byte Burst Read...")
    
    DEVICE_ADDR = 0x68   
    REGISTER    = 0x3B   

    DATA        = [0x03, 0xE8, 0xFF, 0x01, 0x0C, 0x12, 0x1A, 
                   0xBC, 0x00, 0x05, 0x21, 0x02, 0xFF, 0xF0]

    mpu_segments = encode_i2c_transaction(DEVICE_ADDR, REGISTER, DATA)
    
    print_transaction(mpu_segments)
    animate_transaction(mpu_segments, title="MPU6050 Multi-byte Transaction")
